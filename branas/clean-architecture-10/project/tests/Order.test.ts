...
import CurrencyTable from "../src/domain/entity/CurrentyTable";

test("Deve criar um pedido com cpf inválido", async function () {
...
test("Não pode adicionar item duplicado", async function () {
  const uuid = crypto.randomUUID();
  const cpf = "407.302.170-27";
  const order = new Order(uuid, cpf);
  order.addItem(new Product(1, "A", 1000, 100, 30, 10, 3, "BRL"), 1);
...
test("Deve criar um pedido com 3 itens sendo um em dólar", async function () {
  const uuid = crypto.randomUUID();
  const cpf = "407.302.170-27";
...
test("Deve criar um pedido e gerar o código", async function () {
  const uuid = crypto.randomUUID();
  const cpf = "407.302.170-27";
